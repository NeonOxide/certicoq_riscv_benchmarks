...
module Coq_Pos =
 struct
  ...
  let rec of_hex_uint_acc d0 acc =
    ...
    | D12 l ->
      of_hex_uint_acc l (add (XO XH) (mul (XO (XO (XO (XO XH)))) acc))
    | D13 l ->
      of_hex_uint_acc l (add (XI XH) (mul (XO (XO (XO (XO XH)))) acc))
    | D14 l ->
      of_hex_uint_acc l (add (XO (XO XH)) (mul (XO (XO (XO (XO XH)))) acc))
    | D15 l ->
      of_hex_uint_acc l (add (XI (XO XH)) (mul (XO (XO (XO (XO XH)))) acc))
    | D16 l ->
      of_hex_uint_acc l (add (XO (XI XH)) (mul (XO (XO (XO (XO XH)))) acc))
    | D17 l ->
      of_hex_uint_acc l (add (XI (XI XH)) (mul (XO (XO (XO (XO XH)))) acc))
    | D18 l ->
      of_hex_uint_acc l
        (add (XO (XO (XO XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | D19 l ->
      of_hex_uint_acc l
        (add (XI (XO (XO XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | Da l ->
      of_hex_uint_acc l
        (add (XO (XI (XO XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | Db l ->
      of_hex_uint_acc l
        (add (XI (XI (XO XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | Dc l ->
      of_hex_uint_acc l
        (add (XO (XO (XI XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | Dd l ->
      of_hex_uint_acc l
        (add (XI (XO (XI XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | De l ->
      of_hex_uint_acc l
        (add (XO (XI (XI XH))) (mul (XO (XO (XO (XO XH)))) acc))
    | Df l ->
      of_hex_uint_acc l
        (add (XI (XI (XI XH))) (mul (XO (XO (XO (XO XH)))) acc))
  ...
  | UIntDecimal d1 -> of_uint d1
  | UIntHexadecimal d1 -> of_hex_uint d1

  (** val of_int : signed_int -> positive option **)
  ...
  (** val of_hex_int : signed_int0 -> positive option **)
  ...
  (** val of_num_int : signed_int1 -> positive option **)

  let of_num_int = function
  | IntDecimal d1 -> of_int d1
  | IntHexadecimal d1 -> of_hex_int d1
  ...
  (** val to_num_hex_uint : positive -> uint1 **)

  let to_num_hex_uint n0 =
    UIntHexadecimal (to_hex_uint n0)

  (** val to_int : positive -> signed_int **)

  let to_int n0 =
    Pos (to_uint n0)

  (** val to_hex_int : positive -> signed_int0 **)

  let to_hex_int p =
    Pos0 (to_hex_uint p)

  (** val to_num_int : positive -> signed_int1 **)

  let to_num_int n0 =
    IntDecimal (to_int n0)

  (** val to_num_hex_int : positive -> signed_int1 **)

  let to_num_hex_int n0 =
    IntHexadecimal (to_hex_int n0)
  ...
  | PeanoOne -> f
  | PeanoSucc (p, p1) -> f0 p p1 (coq_PeanoView_rect f f0 p p1)
  ...
  | PeanoOne -> f
  | PeanoSucc (p, p1) -> f0 p p1 (coq_PeanoView_rec f f0 p p1)
...
module Make =
 ...
 struct
  ...
  let partition f s =
    let p = Raw.partition f s in Pair ((fst p), (snd p))
      ...
      | Nil ->
        (match delta with
         ...
         | Cons (p, l0) ->
           ...
           (match l0 with
            | Nil ->
              mergeC expr_cmp expr_cmp (Obj.magic id) s t0 (cclose_aux l')
            | Cons (_, _) -> Nil))
      | Cons (_, _) -> Nil)
   | _ -> Nil)
         ...
         Cons
           ((mkPureClause
              ...
              (insert_uniq pure_atom_cmp (norm_pure_atom (Eqv (u, t0)))
                (merge pure_atom_cmp (rev1 pos0) pos))),
  ...
  let rec main_terminate n0 units l =
    ...
    | False ->
      (match existsb is_empty_clause (map delete_resolved l) with
       ...
       | False ->
         let Pair (l0, l1) = partition is_unit_clause l in
         (match partial_mod Nil Nil
                  (filter not_taut
                    (map
                      (simplify (DebuggingHooks.print_eqs_list (cclose l0)))
                      l1)) with
          | Inl p ->
            let Pair (m, l2) = p in
            Pair ((Pair ((Pair ((C_example (m, (clause_list2set l2))),
            (cclose (app (cclose l0) units)))),
            (clause_list2set
              (filter not_taut
                (map (simplify (DebuggingHooks.print_eqs_list (cclose l0)))
                  l1))))), M.empty)
          | Inr p ->
            let Pair (p0, c0) = p in
            let Pair (_, c1) = p0 in
            main_terminate (Coq_Pos.pred n0) (cclose (app (cclose l0) units))
                  ...
                  (app
                    (infer c0 c1
                      (filter not_taut
                        (map
                          (simplify
                            (DebuggingHooks.print_eqs_list (cclose l0))) l1)))
                    (filter not_taut
                      (map
                        (simplify (DebuggingHooks.print_eqs_list (cclose l0)))
                        l1)))))))
       ...
       | False ->
         ...
         (match partial_mod Nil Nil
          ...
          | Inr p ->
            let Pair (p0, c0) = p in
            let Pair (_, c1) = p0 in
            main (Coq_Pos.pred n0) (cclose (app (cclose l0) units))
              (DebuggingHooks.print_pures_list
                (rsort (rev_cmp compare_clause2)
                  (app
                    (infer c0 c1
              ...
              | Some y ->
                Cons
                  ((insert (rev_cmp compare_space_atom) (Lseg (x0, y))
                     (insert (rev_cmp compare_space_atom) (Lseg (y, Nil2))
                       (app (rev1 sigma0) sigma2'))),
                  (unfolding3' (Cons ((Lseg (x0, Nil2)), sigma0)) sigma1
                    sigma2'))
              | None ->
                      ...
                      (DebuggingHooks.print_wf_set
                        (HeapResolve.do_wellformed
                          (DebuggingHooks.print_spatial_model
                            (HeapResolve.norm
                              (DebuggingHooks.print_wf_set t1)
                              (PosSpaceClause (Nil, Nil,
                              (Superposition.simplify_atoms l sigma)))) m)))
                      t0) t0) with
          | True ->
            (match Superposition.is_model_of_PI (rev1 m)
                     (DebuggingHooks.print_spatial_model
                       (Superposition.simplify l nc) m) with
             | True ->
               (match isEq
                        (M.compare
                          (incorp
                              ...
                              (HeapResolve.do_wellformed
                                (DebuggingHooks.print_spatial_model
                                  (HeapResolve.norm
                                    (DebuggingHooks.print_wf_set t1)
                                    (PosSpaceClause (Nil, Nil,
                                    (Superposition.simplify_atoms l sigma))))
                                  m))) t0)
                          (incorp
                  ...
                  the_loop_terminate (Coq_Pos.pred n0)
                    ...
                    (incorp
                      ...
                      (incorp
                        ...
                        t0))
                    (DebuggingHooks.print_spatial_model
                      (HeapResolve.norm (DebuggingHooks.print_wf_set t1)
                        (PosSpaceClause (Nil, Nil,
                        (Superposition.simplify_atoms l sigma)))) m))
             | False -> C_example m)
          | False ->
                ...
                (DebuggingHooks.print_wf_set
                  (HeapResolve.do_wellformed
                    (DebuggingHooks.print_spatial_model
                      (HeapResolve.norm (DebuggingHooks.print_wf_set t1)
                        (PosSpaceClause (Nil, Nil,
                        (Superposition.simplify_atoms l sigma)))) m))) t0)
              (DebuggingHooks.print_spatial_model
                (HeapResolve.norm (DebuggingHooks.print_wf_set t1)
                  (PosSpaceClause (Nil, Nil,
                  (Superposition.simplify_atoms l sigma)))) m))
       | Superposition.Aborted l0 -> Aborted (l0, cl))
         ...
         (match isEq
          ...
          | True ->
            (match Superposition.is_model_of_PI (rev1 m)
             ...
             | False -> C_example m)
          | False ->
            the_loop (Coq_Pos.pred n0) (Superposition.simplify_atoms l sigma)
              (Superposition.simplify l nc)
              (incorp
                (DebuggingHooks.print_wf_set
                  (HeapResolve.do_wellformed
