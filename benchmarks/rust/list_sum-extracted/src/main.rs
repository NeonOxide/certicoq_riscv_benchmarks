...
fn Coq_Lists_List_repeat<A: Copy>(&'a self, x: A, n: u64) -> &'a Coq_Init_Datatypes_list<'a, A> {
  __nat_elim!(
    {
      self.alloc(
...
fn Coq_Lists_List_repeat__curried<A: Copy>(&'a self) -> &'a dyn Fn(A) -> &'a dyn Fn(u64) -> &'a Coq_Init_Datatypes_list<'a, A> {
  self.closure(move |x| {
    self.closure(move |n| {
      self.Coq_Lists_List_repeat(
        x,
...
fn CertiCoq_Benchmarks_rust_tests_list_sum(&'a self) -> u64 {
  self.Coq_Lists_List_fold_left(
